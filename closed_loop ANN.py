...
def main():
    ...
    theta_ref = 0
    ocp.cost.yref  = np.hstack((norm_output((0,theta_ref), norm)))#np.zeros((ny, ))
    ocp.cost.yref_e = np.hstack((norm_output((0,theta_ref), norm)))#np.zeros((ny_e, ))
    ...
    Nsim = 100
    # reference = randomLevelReference(Nsim+N_horizon, [10,15], [-3.1,3.1])
    ...
    # reference = np.hstack((np.ones(200)*3.0))
    ...
    for i in range(Nsim):
        for j in range(1,N_horizon):
            acados_ocp_solver.cost_set(j, 'yref', np.hstack((norm_output((0,reference[i+j]), norm))))
        acados_ocp_solver.cost_set(N_horizon, 'yref', np.hstack((norm_output((0,reference[i+N_horizon]), norm))))
